...
- 模型的尺寸/比例/分辨率/时长等参数选项，只展示该模型上游实测支持的档位，不支持的一律隐藏或置灰并自动回落，避免误选误扣费；新增模型上架前必须先实测参数支持矩阵（各模型矩阵记录见 docs/agent/ 内部文档）。
- 模型下拉未登录也展示研路真目录（与登录后同一份）；点生成仍弹登录。标签只写产品名，不跟「（研路AI）」或「（默认渠道）」；生图和视频介绍必须带上用户实际支付的账本$价格。
- 批量生成、多图展示、助手面板等画布交互要尽量简洁，不要占用过多画布空间。
- 图生视频（Grok/Seedance，`grokVideoApi`）有参考图时不要发 `aspect_ratio`（官方带了会 stretch），视频框跟原图比例；文生视频仍带用户选的比例。不要发像素 `size`。图连在文字上、从视频节点生成时也要沿文字再收一层首帧（只发第一张 `image.url`），不要沿图片无限穿透，不要接 `reference_images`。
...
- 架构改动（换门、改鉴权/计费归属、动全站请求路径）且当前主模型不是 Fable 5 时：先派只读子代理、模型用 `claude-fable-5-thinking-max`（Fable 5 Max）。**Fable 只做裁决，不翻仓库。** 主模型（便宜模型）先读完代码，把关键证据做成「文件:行号 + 短摘录」写进提问，并写明不要再搜、不要再读文件。只问行不行、哪里会断、附哪些条件。未获 GO / 附条件 GO 之前不许动手。实现仍由本会话主模型做。
- 用户报故障先按「专业工程流程」：复现 → 对照隔离 → 根因要运行证据 → 第一性原理修复 → 用同一条路验证。没复现、没证据不许改代码。生图/视频故障仍必须核四方。
- 生图/视频成功 = 浏览器能打开的字节（近节点/出图门/本地 blob）。境外 CDN 回链（如 x.ai）不算成功，不许当完成态。Grok 图请求 `b64_json`；Grok/托管视频有任务 id 时优先 `GET /videos/{id}/content`。CPA 的 content 现网 404，近节点必须在 content 不是真片子时，从 status 里的 `*.x.ai` 地址自己拉成片再回给浏览器。本轮不把 Grok 成片写入 R2 / `img.flyli.cn`。远程回链没有 CORS、没有本地字节时，不许按 0×0 缩放节点，不许造没有 blob 的 `storageKey`。
- 查模型参数、成图尺寸/比例、或计费档位对不上时，必须同时核这四方，缺一停下来补，不许用单点猜：①画布实际 POST 的字段（不要只看药丸文案）、节点是否按原图像素缩放、`readImageMeta` 超时/失败有没有写成 1024×1024；②本仓已钉契约（`docs/agent/STATE.md`、pending-test、CHANGELOG、历史「翻译层丢参」）；③反代有没有丢掉/改写参数——近节点 gen-api、img-api、CPA/cliproxy，画布发出去 ≠ 上游收到；④官方字段与实测档位（Grok 图只认 `aspect_ratio` + `resolution` 的 `1k|2k`，`size` 会被无视）。官方不支持的不要怪画布；官方支持但反代没传的不要怪官方。结论必须写清责任在哪一层或叠加。Grok 生图请求必须自带 `aspect_ratio` + `resolution`，不要只发像素 `size`。Grok/Seedance 图生视频有参考图时不要发 `aspect_ratio`，框跟原图比例；图连在文字上、从视频节点生成时也要收到首帧。
