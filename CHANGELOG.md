...
+ [修复] Grok 图生视频成片已生成时，近节点取回失败会再试几次 `/content`，不再第一次失败就报打不开境外地址

## v0.17.13 - 2026-08-31

+ [修复] GROK 2.0 白板连多张参考图改图时按官方 `images` 字段提交，不再报 image is required

## v0.17.12 - 2026-08-25

+ [修复] 图生视频有参考图时不再发 aspect_ratio，避免成片被 stretch 拉扁，视频框跟原图比例
+ [修复] 图只连在文字上时，从空视频节点生成也会带上首帧图

## v0.17.11 - 2026-08-24

+ [修复] 境内打不开境外成片地址时：Grok 图改走近节点回包里的图片字节，Grok 视频优先从近节点取成片，不再把打不开的回链当成成功
+ [修复] Grok 生图回链无法落入本地时，节点不再缩成看不见，也不再留下空的本地钥匙
+ [修复] 远程视频/图片下载不再走 FileSaver 同步探测，避免一点下载整页卡死；播不了的视频给出「新标签查看」

## v0.17.10 - 2026-08-24

+ [修复] GROK 生图按官方字段提交比例和 1K/2K，9:16(2k) 不再交成默认方图
+ [修复] 生图改走 URL 后图生图卡住：GROK 按官方 JSON 传参考图链接，不再在浏览器里跨域拉字节
+ [调整] GROK 视频按画质标价：低 $0.6 / 中 $0.8 / 高 $1.5 每秒
+ [调整] Seedance 介绍按画质标价，清晰度药丸改为低/中/高
+ [调整] GPT 生图 $2.3/张，GROK 2.0 为 1K $1.2 / 2K $2.2

## v0.17.9 - 2026-08-24

+ [调整] 未登录也展示完整模型目录，点生成仍需登录
+ [调整] 模型名不再带「（研路AI）」后缀
+ [调整] 生图和视频模型介绍补上价格

## v0.17.8 - 2026-08-24

+ [调整] Grok 生图只保留 2.0，收掉快速/写实/旧改图
+ [新增] 上架聊天模型 Grok 4.6
+ [调整] Grok 视频按 1.5 正式名转发，不再改写成 preview
+ [调整] 模型下拉改按社区/官方口径命名，去掉易撞车的「高质量」
+ [调整] Seedance 按官网补齐未贴上的档：2.0 1080 与 2.5 全档
+ [修复] 新建画布节点不再叠在当前选中节点上
+ [修复] 连上参考图时自动切到可改图模型
+ [修复] 生图下拉切换能力后不再残留视频模型
+ [优化] 生图回包改走 URL，避免大图整包超时

## v0.17.7 - 2026-08-24

+ [新增] 上架 Grok 生图 2.0，可调低/中画质，支持改图
+ [新增] 上架 Seedance 2.0 / 2.5 视频，按时长与清晰度单独计价

## v0.17.6 - 2026-08-24

+ [修复] 组装提示词未写 @ 引用时自动携带全部已连接内容，不再静默丢弃

## v0.17.5 - 2026-08-24

+ [优化] 模型参数选项收敛为能力矩阵驱动，Grok 隐藏不支持的透明背景

## v0.17.4 - 2026-08-24

+ [调整] GPT 生图改为单一档位 $1/张，隐藏上游不存在的 2K/4K
+ [修复] GPT 生图比例选择真实生效

## v0.17.3 - 2026-08-24

+ [修复] Grok 视频分辨率/比例/声音开关真实生效，支持 1080p
+ [修复] Grok 视频时长上限对齐官方 15 秒
+ [调整] Grok 图不再发送无效的质量参数

## v0.17.2 - 2026-08-24

+ [调整] 模型显示名按产品口径更新
+ [修复] Grok 图隐藏上游不存在的 4K 档

## v0.17.1 - 2026-08-24

+ [新增] 上架 Grok 高质量图
+ [调整] 模型改用中文显示名并附能力说明
+ [修复] Grok 视频 1.5 对齐官方正式版
+ [调整] 对用户隐藏版本号与版本更新弹窗

## v0.17.0 - 2026-08-23

+ [修复] 登录后刷新页面仍能恢复研路AI渠道，新配置节点不再误显示默认渠道。
+ [修复] 登录后新画布默认使用托管模型，无需手动切换
+ [修复] 生成结束后顶栏余额及时刷新
+ [新增] 生成请求切换至亚洲近节点入口，Grok 图像/改图/视频与文本直达车间更快。
+ [优化] 出图预扣改为按档位金额，低余额可正常生成小图。
+ [修复] 登录开通只保留一份托管渠道配置。
+ [调整] 图片生成默认 1 张。
+ [修复] 托管视频默认改为 grok-imagine-video-1.5，生成失败改为中文提示。
+ [调整] 设置里去掉自定义渠道入口，登录后只使用研路AI托管服务，不再填写接口地址或密钥。
+ [调整] 登录/充值与 Grok、文本生成入口拆开，生成为国内节点预留独立地址；GPT Image 2 仍走出图接口。
+ [调整] 登录后强制开通托管密钥 `RIGEL-图像` / `RIGEL-文本`，分别绑画布图像分组和稳定 Codex 文本分组，不再占用用户自己改过的「画布」密钥。
+ [修复] 生图失败时展示上游返回的真实原因，不再笼统提示「请简化描述」。
+ [新增] 创作画布上线 `https://canvas.flyli.cn`。
+ [调整] 研路托管 Grok 模型只保留独立能力与最新版：生图 `grok-imagine-image`、图片编辑 `grok-imagine-edit`、视频 `grok-imagine-video-1.5`；去掉同功能的 quality / 旧版视频 / 1.5 preview。
+ [调整] 隐藏顶栏 GitHub 与 Agent 连接入口，画布不再自动打开 Agent 面板。
+ [调整] 下线画布语音/TTS 生成：不再提供音频模型、语音设置、音频节点生成和 Agent 音频工具；视频「生成声音」开关保留。
+ [新增] 研路AI托管渠道接入 GPT 5.6 文本模型（sol / luna / terra），登录后用账号自己的「画布-文本」密钥扣费。
+ [调整] GPT Image 2 改为走研路自有出图通道，Grok 生图仍走现有研路接口；两边都由托管账号出图。
+ [调整] GPT 生图排队提示改为显示前面还剩几张，并随服务端真实队列递减；轮到下一位时提示下一个就轮到你。
+ [新增] 全站提示作品只保存在当前浏览器，请及时下载；更换设备或清理缓存后本地数据会消失。
+ [调整] 研路AI托管渠道生图下拉新增 Grok Image 与 Grok Image 高质量，视频下拉新增 Grok 图生视频及 1.5 预览；默认生图仍为 GPT Image 2。
+ [新增] 支持研路AI账号登录（含人机验证与两步验证），登录后自动开通「画布」生图密钥并接入托管渠道，无需手动配置。
+ [新增] 顶栏新增账号入口，显示用户名与美元余额，支持充值跳转与退出登录，余额在生成结束和窗口聚焦时自动刷新。
+ [新增] 生图任务展示排队位次、预计耗时与自动重试倒计时；余额不足时提供去充值入口。
+ [调整] 生成/编辑动作无条件要求登录：未登录时一律弹出研路AI登录框，即使本地已配置可用渠道；本地渠道登录后仍可使用。
+ [调整] 配置页不再展示托管渠道的接口地址与密钥，也不可编辑或删除该渠道。
